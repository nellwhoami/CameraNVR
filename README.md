...
主要是tp-link监控的一些功能需要付费,穷屌丝只有自己撸一下代码,顺便白嫖百度网盘的资源.

# 主要功能

1，视频捕获：通过提供的视频流URL，该程序可以捕获实时视频流。

2，运动检测：使用YOLO3 模型识别人行,还算比较准确吧,目前没发现什么问题,yolo3 的三个文件下载地址: aHR0cHM6Ly9wYW4uYmFpZHUuY29tL3MvMTd2TldpQjB1QjdjR182ODhZUEhiTVE/cHdkPTY2NjY=  ,解压到py文件同目录

3，视频录制：当程序检测到运动时，会开始录制视频，并将包含运动物体的视频段保存在本地

4，视频上传：根据设置，程序录制的视频上传到指定的网盘

5, 实时推送监控日志到手机,BARK_API_KEY = ["key111","key222"]# Bark推送的KEY ,一个手机就填一个就好了
